        ...
        try:
            for dados_dict in dados_list:
                if dados_dict.get('id_usuario'):
                    usuario = self.sessao.query(Usuario).filter_by(id_usuario=dados_dict['id_usuario']).first()
                    if usuario:
                        usuario.ativo = dados_dict['ativo']
                        usuario.username = dados_dict['username']
                        usuario.senha = dados_dict['senha']
                else: 
                    usuario_insert = Usuario(username=dados_dict['username'], senha=dados_dict['senha'], ativo=dados_dict['ativo'])
