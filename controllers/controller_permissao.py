        ...
        try:
            for dados_dict in dados_list:
                if dados_dict.get('id_permissao'):
                    permissao = self.sessao.query(Permissao).filter_by(
                        id_permissao=dados_dict['id_permissao']).first()
                    if permissao:
                        permissao.nivel_permissao = dados_dict['nivel_permissao']
                        permissao.id_usuario = dados_dict['id_usuario']

                else:
                    permissao_insert = Permissao(
                        nivel_permissao=dados_dict['nivel_permissao'], id_usuario=dados_dict['id_usuario'])
